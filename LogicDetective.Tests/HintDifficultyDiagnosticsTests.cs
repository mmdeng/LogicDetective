...
public sealed class HintDifficultyDiagnosticsTests
{
    ...
    public void Run_GeneratedPuzzleHasValidMinimumClueCount()
    {
        ...
        Assert.All(report.Measurements,
            measurement =>
            {
                Assert.True(measurement.ClueCount > 0);
                Assert.True(measurement.MinimumClueCount > 0);
                Assert.True(measurement.MinimumClueCount <= measurement.ClueCount);
                Assert.Equal(measurement.ClueCount - measurement.MinimumClueCount, measurement.RedundantClueCount);
            });
    }
    [Fact]
    ...
    {
        var measurements = Array.Empty<HintDifficultyMeasurement>();
        Assert.Throws<ArgumentException>(() => HintDifficultyDiagnostics.Summarize(measurements));
    }
    ...
    {
        var categories = CreateCategories();
        Assert.Throws<ArgumentOutOfRangeException>(() => HintDifficultyDiagnostics.Run(categories, 0));
    }
    ...
    {
        var categories = CreateCategories();
        var report = HintDifficultyDiagnostics.Run(categories, 5);
    ...
    public void Run_GeneratedPuzzlesHaveNoRedundantClues()
    {
        ...
        Assert.All(report.Measurements,
            measurement =>
            {
                Assert.Equal(measurement.ClueCount, measurement.MinimumClueCount);
                Assert.Equal(0, measurement.RedundantClueCount);
            });
    }
    ...
    private static CategoryList CreateCategories()
    {
        return
        [
            new Category(0, "Days", ["Monday", "Tuesday", "Wednesday"]),
            new Category(1, "People", ["A", "B", "C"]),
            new Category(2, "Food", ["Curry", "Ramen", "Sushi"])
        ];
    }
    ...
    //     return new Item(categoryIndex, index, names[categoryIndex][index]);
    // }
}